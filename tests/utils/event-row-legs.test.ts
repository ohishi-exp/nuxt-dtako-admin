...
describe('assignRowsToLegs', () => {
   ...
   * この 19 行は**その規則の検査用に組んだ形**であって、本番の CSV そのものではない
   * (下の訂正を参照。**実データに差し替えない** — 差し替えると規則の意図が読めなくなる)。
   *
   * ## ★ 訂正 (2026-08-25、Refs #871) — 「19 行」は**本番の CSV ではなかった**
   *
   * この doc には元々こう書いてあった (`d1e5493`):
   *
   ...
   *
   * **間違いは「測定条件を書かなかったこと」ではない。条件 (「本物の KUDGIVT.csv」) は
   * 書いてあったのに、そこから引いた結論の適用範囲を間違えた。** 測ったのは
   * `get_operation_zip` で取った theearth の `csvdata.zip` の中身で、**画面が読む
   * 本番 API とは別物**だった。14 行に着くのは同じでも**経路が違う**:
   *
   * ```
   * zip  :  19 行 → event 区分 14 行 (5 行が別タブへ)
   * 本番 : 102 行 → event 区分 99 行 → dropIgnoredRows が「無視」85 行を落として 14 行
   * ```
   *
   * ### 2 つの測定 (どちらも運行 `2607010121120000001318`、2026-07-01)
   *
   * | | theearth `csvdata.zip` (`get_operation_zip`) | **本番 API** |
   * |---|---|---|
   * | 行数 / 列数 | 19 | **102 / 34 列** |
   * | `急加速` / `急減速` / `急カーブ` | 0 本 | **34 / 19 / 32 = 85 本** |
   * | `一般道空車` | 2 本 (51.3km / 0.1km) | **0 本** |
   * | 運転 / 積み / 降し / 専用道 / 速度オーバー / アイドリング / 休憩 | 6 / 2 / 3 / 1 / 1 / 1 / 1 | 同じ |
   *
   * 本番側の測定条件: `/api/proxy/api/operations/2607010121120000001318/csv/events`、
   * v0.0.543、ブラウザから、`対象乗務員区分` は `"1"` のみ。
   * 85 本の 急加速・急減速・急カーブ は**すべて 区間距離 0 かつ 区間時間 0** なので
   * `dropIgnoredRows` の「数字を持つ行は隠さない」に引っかからず全部落ちる。
   *
   * **`一般道空車` が zip に 2 本 (51.3km / 0.1km) あって本番 API に 0 本**なのは
   * 事実として書き残すだけ (原因は #871 の対象外)。`totalKm` / `overlayKm` を
   * zip と本番で突き合わせると**この 51.4km ぶん食い違う**ので、先に条件を疑うこと。
   *
   * ### 本番の主経路 (102 → 99 → 14) を固定しているのはこのファイルではない
   *
   * `dropIgnoredRows` が絡む主経路は `tests/components/EventDataTable.test.ts` の
   * 「本番 1318 の主経路」に置いた (画面の状態 = `showIgnored` が要るため)。
   * ここは pure 関数の規則だけを見る。
   */
