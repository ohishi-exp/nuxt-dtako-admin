...
const DRIVERS: WageMixInput[] = [
  { driverCd: '1412', driverName: '中村　一由', allowanceBaseYen: 609000, restraint: { workingMinutes: 18974, overtimeMinutes: 6397, nightMinutes: 918, workDays: 27 } },
  { driverCd: '1587', driverName: '柳井　亮祐', allowanceBaseYen: 552500, restraint: { workingMinutes: 15517, overtimeMinutes: 4611, nightMinutes: 7884, workDays: 23 } },
  { driverCd: '1656', driverName: '西島　健太', allowanceBaseYen: 542500, restraint: { workingMinutes: 12681, overtimeMinutes: 4312, nightMinutes: 883, workDays: 18 } },
  { driverCd: '1732', driverName: '佐竹　繁', allowanceBaseYen: 561000, restraint: { workingMinutes: 19689, overtimeMinutes: 6760, nightMinutes: 2015, workDays: 27 } },
  { driverCd: '1742', driverName: '増地　誠', allowanceBaseYen: 499000, restraint: { workingMinutes: 18919, overtimeMinutes: 5962, nightMinutes: 1493, workDays: 27 } },
]
...
describe('computeWageMixRow — 方式B (日数) は資料の数字を再現する', () => {
  ...
  it('現状の推計 (母数 + 一律残業代) が資料の支給額と一致する (佐竹だけ過払い ¥1,000 ぶん少ない)', () => {
    expect(rows.map(r => r.currentYen)).toEqual([619000, 562500, 552500, 571000, 509000])
  })
...
describe('computeWageMixRow — 割増の係数', () => {
  const one = (r: Partial<WageMixInput['restraint']> & { workDays: number }): WageMixInput => ({
    driverCd: '1', driverName: 'x', allowanceBaseYen: 0,
...
describe('computeWageMixRow — 欠測を 0 に倒さない', () => {
  const noRestraint: WageMixInput = { driverCd: '1999', driverName: '拘束なし', allowanceBaseYen: 100000, restraint: null }
  ...
  it.each([
    ...
    const row = computeWageMixRow(
      { driverCd: '1', driverName: 'x', allowanceBaseYen: 0, restraint: { ...restraint, workDays: 10 } },
      settings(),
    )
    expect(row.totalYen).toBeNull()
...
describe('summarizeWageMix', () => {
  it('方式B の合計が資料の 旅費 ¥967,400 / 基本給 ¥1,049,200 と一致する', () => {
    ...
    expect(totals).toMatchObject({
      ...
      overtimeYen: 687117, totalYen: 2703717, currentYen: 2814000, diffYen: -110283,
      workDays: 122,
    })
  })
  ...
  it('欠測の乗務員は合計に入れず、名前と旅費を別に持つ', () => {
    const rows = [
      ...
      computeWageMixRow({ driverCd: '1999', driverName: '拘束なし', allowanceBaseYen: 100000, restraint: null }, settings()),
    ]
  ...
  it('1 人も居なければ全部 0', () => {
    expect(summarizeWageMix([])).toEqual({
      drivers: 0, computed: 0, missingDrivers: [], allowanceBaseYen: 0, travelYen: 0,
