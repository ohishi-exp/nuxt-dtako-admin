...
/**
 * 釧路区画の金額に **`¥-0` を出さない** (Refs #843 / #928)。
 *
 * `fmtYen` は粗利 (試算) / 営業利益 (試算) にも使う。どちらも負になり、`Math.round` は
 * `-0.5 ≤ v < 0` で **`-0`** を返し、**`(-0).toLocaleString()` は `"-0"`** なので、
 * その回に `¥-0` と出ていた。**「0 円」なのか「符号が化けた」のか読めない**のが害。
 *
 * **陽性対照を必ず置く** — `Math.abs` のように符号ごと消す直しでも通ってしまう
 * テストにしないため。
 */
describe('fmtYen が `¥-0` を出さない (Refs #843)', () => {
  ...
  it('陽性対照: 本当に負の額は負のまま (-0.6 → ¥-1、丸め方を変えていない)', () => {
    expect(fmtYen(-0.6)).toBe('¥-1')
  })

  it('陽性対照: まとまった負の額も消えない (Math.abs で潰していない)', () => {
    expect(fmtYen(-50000)).toBe('¥-50,000')
    expect(fmtYen(-1234.5)).toBe('¥-1,234')
  })

  it('正の額・null は 1 文字も変わらない', () => {
    expect(fmtYen(50000)).toBe('¥50,000')
    expect(fmtYen(0.4)).toBe('¥0')
    expect(fmtYen(null)).toBe(DASH)
  })

  it('fmtSignedYen は元から守られている (Math.abs 通し。当てていないことの記録)', () => {
    expect(fmtSignedYen(-0)).toBe('+¥0')
    expect(fmtSignedYen(-0.4)).toBe('−¥0')
    expect(fmtSignedYen(-0.6)).toBe('−¥1')
  })
})
