...
import rawIdles from '../fixtures/kushiro-loading/deadhead-idle-2026-07.json'
import measured from '../fixtures/kushiro-loading/measured-2026-07.json'
import golden from '../fixtures/kushiro-loading/golden/summary-2026-07.json'
...
describe('釧路積みの判定', () => {
  it('手当マスタの語彙 (origin = 釧路) をそのまま渡しても当たる', () => {
    expect(RATE_MASTER.some(row => row.origin === KUSHIRO_ORIGIN)).toBe(true)
  ...
  it('粗利タブの OperationMargin をそのまま渡せる (新しい入力型を発明していない)', () => {
    // 代入できることを tsc に確かめさせるのが本体。実行時は関数があるだけ。
    const asInput: (op: OperationMargin) => KushiroOperationInput = op => op
    expect(typeof asInput).toBe('function')
  })

  it('釧路積みの業者はマスタから導く (実住所 3 種 = 3 社ぶんある)', () => {
    expect(KUSHIRO_LOADERS.length).toBeGreaterThanOrEqual(3)
    for (const loader of KUSHIRO_LOADERS) {
      expect(RATE_MASTER.some(row => row.origin === KUSHIRO_ORIGIN && row.loader === loader)).toBe(true)
    }
  })
})
...
const GOLDEN_PATH = 'tests/fixtures/kushiro-loading/golden/summary-2026-07.json'
...
describe('golden (共有 fixture の出力を丸ごと固定)', () => {
  ...
  if (UPDATE_GOLDEN) {
    it('golden を再生成した (UPDATE_GOLDEN)', async () => {
      ...
      expect(output.summary.counts.all).toBeGreaterThan(0)
    })
    return
  }
