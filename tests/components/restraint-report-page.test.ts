 ...
 * 2. **0 分を `0:00` と書かない** — `fmt()` は 0 と null をどちらも空にする。
 *    表の空欄は「その区分の時間が無い」であって「読めなかった」ではない
...
describe('/restraint-report 表の組み立て', () => {
  ...
  it('休息時間があれば出す (null と 0 を同じ空欄にするのは現状の挙動)', async () => {
  ...
  it('★ 0 分は 0:00 と書かず空欄 (「無い」と「読めなかった」を分けない代わりに揃える)', async () => {
    const w = await show(report({
      days: [day('2026-07-06', {
