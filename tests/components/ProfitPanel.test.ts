...
describe('ProfitPanel', () => {
  ...
  describe('明細の表示', () => {
     ...
     * 本番 2026-07 の候補一覧 (**その便だけを選んだ状態**で実測) では 1,668 件中 455 件
     * (27.3%) がこの状態で、候補が 2 件以上ある便 380 本のうち **41 本**がバッジ一色に
     * なっていた。畳んだ側を 1 行で言わせる。**`location` は選択区間の積地・卸地なので、
     * 選択の取り方で分布はまるごと変わる** — 測り直すときは選択の条件を必ず書く。
     */
