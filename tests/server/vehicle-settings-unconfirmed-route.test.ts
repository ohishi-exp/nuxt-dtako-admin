 ...
 * 転送し、上流 auth-worker `/alc-proxy` が token 不在を 401 にする。
 * **⇒ 無防備ではなく、入れる前も車輛マスタは無認証の呼び出し元には返っていない。**
 * それでも A 段へ上げるのは、① B 段の防御が**上流の実装依存**でこの repo からは
 * 保証できない、② **上流 401 を待つ前に R2 list が走ってしまう** (handler の
 * `Promise.all` は listing を並行で回す) の 2 点。
 *
 * ② は使い捨て probe で実測した — **`origin/main` a05bd08 の実装に「上流が 401」の
 * mock を当てると、空バケツ (objects 0 件) で `r2.list` が 1 回呼ばれる**
 * (陽性対照: 上流 200 でも 1 回)。**「未ログインなら 0 回」ではなかった。**
 * 下の「★ 未ログインは 401 で、R2 list も上流フェッチも 1 度も起きない」が
 * その 1 回を 0 回にしたことを固定する。
 *
