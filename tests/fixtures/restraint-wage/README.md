...
| `timecard-daily-2026-06.json` | **タイムカード日別 JSON の実機応答**を 4 名分そのまま切り出したもの (Refs #424 PR-B)。上の合成 fixture と違い**実データ**なのは、休憩の導出 (中抜け ∪ 12:00-13:00) と日跨ぎが実際の打刻の形でしか検証できないため。1029 冨田 (乗務員・日曜出勤3日・日跨ぎ12日・中抜け4日) / 1621 西山 (事務パート層・素直な形) / 1663 松山 (中抜けが昼をまたがない) / 1670 松永 (中抜けが昼をまたぐ = 二重控除してはいけない) | `workers/dtako-scraper-relay/test/timecard-summary-golden.test.ts` |
| `golden/timecard-summaries.json` | `summarizeTimecardMonth` の出力 golden — **手で編集しない** | 同上 |
