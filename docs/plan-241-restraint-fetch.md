...
- **最終確認日**: 内容不変の再取得では latest の `lastVerifiedAt` だけ更新 —
  ...
  次版の `fetchedAt`。
- **確認履歴 (`csv/{range}/history.jsonl`)**: 取得のたびに 1 行追記
  (`{"ts","result":"new-version"|"unchanged"|"no-data","sha256","bytes"}`)。
  **「変わっていなかった」という確認結果も時系列で残る** (直近 1000 行で丸め)。
- **データなしの記録**: 「該当データがありません」(途中入社・休職・未集計で正当に
  ありうる) も history に `no-data` 行として残し、乗務員単体取得なら summary 側にも
  `{noData: true}` マーカーを版管理付きで置く (未取得との区別。後にデータが現れたら
  通常の summary が新しい版になる)。フロントの明細・集計 CSV にも「該当データなし」
  行を出す。
