...
- 令和 6 年 3 月告示 (国交省告示 209 号) の運賃表を構造化データ化
  (距離制 / 時間制 × 運輸局 × 車種、荷待ち・荷役対価、
  燃料サーチャージ 5 円刻みテーブル)
- **一次資料調査の結果 (2026-07-10)**: 「交替運転者配置料金 (2マン運行)」は
  告示 209 号に存在しない (官報・通達・解説集・運賃料金適用方参考例・
  全ト協リーフレットを全て確認)。速達割増の「○割」と同様に各事業者が
  運賃料金適用方で独自設定する領域。計算ロジックでは事業者設定額の
  パラメータ (`addTwoManFee`) として受ける
- 実装済み (`app/utils/tariff/`): 九州運輸局の距離制/時間制 (官報 96〜98 頁から
  転記、二重チェック済み)、距離切り上げ規則 (10/20/50km)、休日・深夜割増 2 割、
  特殊車両割増率、待機時間料、積込料・取卸料、利用運送手数料 10%、
  燃料サーチャージ (基準 120 円/L・5 円刻み・円未満切り上げ)。
  他運輸局ブロックは同構造のデータファイル追加で拡張 (発地が九州以外の
  運行が必要になった時点で追加)
