 ...
 *
 * ## 由来は 2 通り。「手で編集しない」は**もう成り立っていない**
 *
 * 62 行のうち **60 行は xlsx からの機械生成** (`obihiro-profit/gen_rate_ts.py`)。
 ...
 *
 * ⇒ **生成器を回して丸ごと上書きすると、この 2 行が黙って消える。**
 * 消えると釧路→駒場→別海 の中継が `unknown` (イレギュラー) に落ちる —
 * `allowance-rate.ts` は**マスタに無い経路を推測しない**ため。上書きしたら手で戻すこと。
 * **生成器をこの repo に置くか / seed を JSON にするかは未決** — #1017 ⑤ の別
 * issue の領分で、ここでは決めない。
 *
