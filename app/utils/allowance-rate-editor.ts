 ...
 *
 * ## ★ 粗利タブのキャッシュの但し書きと**同じ文にしない**
 *
 * `allowanceRateNoticeForMargin` (PR-2) が粗利タブに足している
 * 「表示中の金額は**前回の集計 (キャッシュ)** のもので、この版で計算したとは
 * 限りません」と**理由が違う**:
 *
 * | | 出るとき | なぜ金額が古いか |
 * |---|---|---|
 * | 粗利タブの但し書き | **キャッシュから復元した回** | キャッシュが**計算済みの金額そのもの**を持っている |
 * | この 1 文 | **保存した回** | 保存の前に引いた金額が表に残っている |
 *
 * 運行手当タブのキャッシュは**金額を残さず読み込み時に引き直す**ので、
 * 「キャッシュだから古い」はこの画面では起きない。1 本の文に寄せると
 * **どちらかの画面で嘘になる** (memory `shared-notice-lies-on-second-screen`)。
 * **揃えたのは「集計 を押すと引き直します」の一手だけ** — やり直し方は同じなので。
 */
