 ...
 * 上士幌・帯広川西・士幌 (十勝) で降ろす運行は帰庫が遠くなる。2026-07 の実測では
 * 卸地が十勝に集中しているので、経路をまとめると十勝側が勝つ — だから
 * **経路 (積地 → 卸地) 単位でも差を出せる形**にしてある (`depotShiftDiff` に
 * `routes[].totals` を渡す)。合計だけ見ると反転が消える。
 *
 ...
 * 数える。混在運行を丸ごと数えると、士幌積み便の回送まで釧路営業所の効果に
 * 入ってしまう。
 *
 * ## 営業所を変えても動かない距離
 *
