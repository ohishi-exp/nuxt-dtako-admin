 ...
 * 変わっていなくても**新しい版が生まれる**。
 *
 * **形式 2 (Refs #886) からは、その設定が指紋として版に入る** — なので「なぜ増えたか」を
 * 後から確かめる術はある。**形式 1 の版には指紋そのものが無い**ので、そちらは確かめられない。
 * **この画面はまだ指紋を突き合わせていない** (差の理由まで言うのは別 PR) ので、
 * どちらの形式でも**断るところまで**は変わらない。**「指紋が入ったから安心」と読ませない。**
...
export const MARGIN_DIFF_OVERRIDE_CAVEAT
  = 'この粗利の差は、集計した端末の設定 (燃費の上書き) でも起こりえます — '
    ...
    + '形式 2 以降の版は端末の設定 (燃費の上書き・運行経費の配分) を指紋として持っていますが、'
    + 'この画面はまだ指紋を突き合わせていません。形式 1 の版には指紋そのものが無いので、'
    + 'そちらは後から確かめる術がありません。'
...
export const MARGIN_DIFF_NO_OPERATION_MARGIN_NOTE
  = '運行 1 本ごとの粗利 (経費・燃料代を引いた額) は出していません — この画面は保存された値を'
    + '引き算するだけで、粗利を計算し直さないためです。形式 2 以降の版は燃費の上書きと運行経費の配分を'
    + '指紋として持っていますが、形式 1 の版には無く、そちらは運行 1 本の粗利を後から厳密に再現できません。'
