 ...
 *
 * ★ **増える条件を決めるのは `marginSummaryHashInput` (`margin-r2.ts`) で、`totals` だけでは
 * ない** — `schemaVersion` と「その版を作った端末の設定」の指紋 (`fuelRateOverrides` /
 * `runCostShareMode`) もハッシュ対象に入る (Refs #886)。**「同じ数字なら版は増えません」と
 * 書いていたら #888 で嘘になった** — 本番 2026-07 で運行・売上・手当・粗利の 4 項目とも
 * 同一の版が 2 本並んだ (形式 1 → 形式 2 の最初の集計、Refs #891)。
 * **ハッシュ対象を変えたら、この文言も一緒に直す。**
 *
 * **逆方向の誤読も潰す** (#854 の型): 「集計すれば必ず増える」と読ませない —
 * 中身が前の版と同じなら `putVersionedProfit` は `changed: false` で本当に増やさない。
 * **どうすれば増えるのかは書くが、指紋の仕組みの説明はしない** — これを読むのは
 * 版が 2 本に足りない人で、要るのは「次の版を作る方法」だけ。
 */
export function marginDiffNeedsMoreVersionsNote(versionCount: number): string {
  ...
  return `R2 に残っている版が ${versionCount} 本しかないので、まだ差分を出せません — 比べるには 2 本以上要ります。`
    + '版が増えるのは「粗利タブで集計して、前の版と中身が変わったとき」です。'
    + '数字が同じでも、集計した端末の設定 (燃費の上書き・運行経費の配分) が変わったときと、'
    + '保存の形式が上がった後の最初の集計では増えます。'
    + '逆に、どれも前の版と同じなら、何度集計しても増えません。'
}
