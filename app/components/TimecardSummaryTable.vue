    ...
    <table class="w-full text-xs monthly-table">
      <thead>
        <tr class="text-left text-gray-500 border-b-2 border-gray-300 dark:border-gray-600">
          ...
          <th class="px-1.5 py-1.5 text-right" title="打刻があり賃金計算に入った日のうち、残業の無い日">通常</th>
          <th class="px-1.5 py-1.5 text-right" title="残業のあった日">残業</th>
      ...
      <tbody>
        <tr v-for="row in rows" :key="row.driverCd" class="border-b border-gray-100 dark:border-gray-800">
          <td class="px-1.5 py-1 whitespace-nowrap">{{ row.driverCd }} {{ row.driverName }}</td>
          <td class="px-1.5 py-1 text-right">{{ fmtDays(row.counts.normal) }}</td>
          <td class="px-1.5 py-1 text-right">{{ fmtDays(row.counts.overtime) }}</td>
