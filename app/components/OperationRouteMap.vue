...
/**
 * 1 運行の経路を**種別ごとに色分け**して Google Map に描くモーダル (Refs #760 の 18)。
 * **便は色ではなく marker の数字で名乗る** (Refs #884。下の `SEGMENT_STYLE` の注記)。
 ...
 * 種別で、便は ▲ / ■ の中の数字)。**イベント表 (`event-row-legs.ts`) は逆で、色 → 便**
 * (`LEG_COLOR_COUNT` の循環) — 同じ色相が 2 つの区画で違う意味を指す (Refs #884)。
 * 下の「ぶつかりを避ける」規則は**地図の中の色だけ**を対象にしていて、表の 5 色は
 * 入っていなかった。**足しても解けない** — 表は 5 色を使い切っていて、地図側で既に
 * 避けている 緑・灰・琥珀・青 と合わせると残る色相がほぼ無い。個別の対応も書けない
 * (表の色は便の本数で循環するので、「表の◯色 = 地図の△色」は運行ごとに変わる)。
 * ⇒ **色は 1 つも変えず、凡例の脚注で「色は便ではない・便は数字」と言い切る**方で解いた
 * (地図はモーダルなので表と同じ画面には並ばない、という実測が根拠)。
 * **次に色を足すときも、避ける相手はこの地図の中の色**でよい。
 *
