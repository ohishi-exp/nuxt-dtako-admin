...
/** `?month=YYYY-MM` (拘束×賃金 の「未同期」丸から飛んでくる、Refs #712) を
 * 既定の対象年月にする。形式が違う/無ければ従来どおり先月 1 ヶ月分。 */
function initialYm(query: unknown, now: Date): string {
  if (typeof query === 'string' && /^\d{4}-\d{2}$/.test(query)) return query
...
onMounted(() => {
  ...
  const ym = initialYm(route.query.month, new Date())
  monthFrom.value = ym
  monthTo.value = ym
})
