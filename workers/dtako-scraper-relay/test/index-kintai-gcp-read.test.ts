...
/** GCP 読み取りの口 (`/kintai-relay/day-summaries` / `calendar-days`) が要る配線だけの env。
...
const CALENDAR = { month: "2026-06", items: [{ driver_cd: 1026, date: "2026-06-24", restraint_minutes: 1792 }] };

describe("GET /kintai-relay/calendar-days (Refs #1123)", () => {
  it("secret が不一致 / 無ければ 401 で、上流を叩かない", async () => {
    for (const headers of [{ "X-Alc-Proxy-Secret": "wrong" }, {}] as Record<string, string>[]) {
      const { env, calls } = fakeEnv(() => Response.json(CALENDAR));
      const res = await worker.fetch(get("/kintai-relay/calendar-days?month=2026-06", headers), env);
  ...
  it("関門を通れば上流の day-parts を month 付きで読み、応答をそのまま返す", async () => {
    const { env, calls } = fakeEnv(() => Response.json(CALENDAR));
    const res = await worker.fetch(get("/kintai-relay/calendar-days?month=2026-06"), env);
    expect(res.status).toBe(200);
    expect(await res.json()).toEqual(CALENDAR);
    expect(calls).toHaveLength(1);
    expect(calls[0]).toContain("/api/kintai/day-parts?month=2026-06");
  });
  ...
  it("上流が落ちたら 502 (古い値に倒さない)", async () => {
    ...
    expect(((await res.json()) as { error: string }).error).toMatch(/calendar-days: status 500: boom/);
  });
...
describe("GET /kintai-relay/day-summaries (関門を calendar-days と共有したあとも同じに動く)", () => {
