 ...
 *    するが、その後の `registration-status` が `resultCode=1699` (変換エラー) を返し、
 *    **予約番号が永久に出ない** (本番実測 2026-08-25、`v0.0.565`)。受付が通るので
 *    「登録できた」ように見えるのが厄介なところ
 ...
 * (2026-08-25、30 運行 → 93 ページ / 10.1 MB、9 運行 → 41 ページ / 3.8 MB)。
 * まとめ版に戻したくなったときのために、その時の実測を残しておく:
 *
 ...
 * - まとめる場合は **netprint の 10MB 上限**に当たる (1 運行 ≈ 0.34 MB ⇒ 約 29 運行が
 *   天井)。1 運行ずつなら当たらない
 *
