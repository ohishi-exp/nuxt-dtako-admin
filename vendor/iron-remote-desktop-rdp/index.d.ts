...
/**
 * RemoteApp (RAIL) として起動するアプリを指定する。省略するとフルデスクトップ。
 *
 * publish 済みアプリのエイリアス (`||ALIAS` 形式) は**配置先ごとの設定**なので、
 * ここでは受け取るだけでコードには持たない。
 */
export declare function remoteApp(program: string): Extension;
