...
- **`riyouMonth{N}` (利用月選択) checkbox は「全選択」の対象から除外する**
  (2026-07-04 実害確認)。検索条件フォームの「全選択」相当ロジック
  (`submitSearch()`) は元々全 checkbox を無差別に選択していたが、ページには
  カード選択等とは別に **利用月ごとの checkbox (`riyouMonth1`/`riyouMonth2`/...)
  があり、ページ既定でチェックされているのは検索対象月 (実機では直近月) の
  みで他は未チェック**。これも「全選択」してしまうと、1回のスクレイプで
  数ヶ月〜1年超分の明細が一括取得される (実害: 25/04〜26/06 の 14 ヶ月分が
  1 CSV に混入)。`RIYOU_MONTH_CHECKBOX_PATTERN` (`/^riyouMonth\d+$/`) に
  一致する checkbox は overrides に含めず、ページ既定のチェック状態
  (`form.fields` にそのまま残る) だけで送る。
