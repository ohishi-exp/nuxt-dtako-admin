...
- **未検証・要確認**: F-GOS0030 の `btnUpdate` 反映フロー全体 (上記で `Return`→
  `ReturnDisplayConfig`→`$('#btnUpdate').click()` の呼び出し連鎖は実機確定したが、
  Worker からの postback 2段階を実際に fetch で通して一覧が絞り込まれることまでは
  未検証)。単一運行ではなく複数選択時の zip 命名規則 (今回のファイル名は
