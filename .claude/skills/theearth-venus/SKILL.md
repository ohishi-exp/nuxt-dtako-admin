...
- **ページャの「...」は窓送りではなく次ページへの直接遷移** (NumericPager 標準)。
  「...」postback の応答から旧ページ番号のリンクを探すと見つからず、**窓の境界ごとに
  harvest が静かに打ち切られる** (実機 2026-07-29: 2ヶ月検索が新しい端の62行だけに
  なった)。応答の `gCurrentPage` が currentPage+1 ならその html を次ページ本体として
  そのまま回収を続けること。また「...」は**戻る側にも出る**ので必ず最後の (進む側の)
  リンクを使う。ページ跨ぎで同一運行が二度読めることがあるため、運行No+開始日時で
  重複排除も必須 (同一運行が一覧に 2 行出た実害あり)
- **窓の先に「...」が無く「次」submit ボタンで進むタイプの窓もある** (「最初」「最後」
  と同じ DataPager ボタン群、実機 2026-07-29: 窓「1 2 3」のみで 3 ページ打ち切り)。
  数値リンク→「...」→「次」ボタンの順で試し、ボタン postback 後は gCurrentPage が
  **増えたことを必ず検証**してから続行する (進んでいなければ最終ページとみなす)。
  次ページ手段が尽きて行ありのまま打ち切る時は、ページ番号/リンク文字列/pager
  ボタン一覧を console.warn で Tail に残す (常設診断)
