...
ような 2 段階 postback / 確認ページは無い。consumer 実装は
`workers/dtako-scraper-relay/src/theearth-restraint-client.ts` (`/restraint-api/*`、
フロントは `/restraint-fetch`)。
...
- **表示条件** (`btnConfig1`、onclick `DataDisplayConfig()`) は
  **`F-GOS0030[DataDisplayConfig].aspx` を `window.open` で開く**、既存の F-GOS0030
  ...
  適用 `btnOK` → 元画面への反映は F-GOS0030 節の3段階フロー — full form 確保→
  GOS0030 差し替え btnOK 適用 postback → 元画面の一覧更新相当 postback — が
  そのまま当てはまるはずだが、F-VOS3020 側の「一覧更新」ボタン ID は未確認・未検証)。
- **行選択**: 他画面 (F-DES1010/F-NOS3010) と同型の CSS 非表示 text input へのクリック
  書き込み方式。**Ctrl+クリックで複数選択でき、カンマ連結される** (`_Multi` 型、
  F-NOS3010 節と同様、実機でブラウザ操作から3行選択して確認済み)。
...
- **既知の不安定さ**: 初回の POST (ダウンロード postback) が**3回連続で HTTP 503**
  を返し、ページには JS エラーハンドラらしき `undefined` とだけ表示された。ページを
  再読み込みして選択からやり直したところ成功した。「黙って200」ではなく明確な
  503 なので検知は容易だが、**リトライで解消することがある**一時的なサーバー不調の
  可能性がある (原因未特定、初回アクセス直後の postback で再現しやすいか等は未検証)。
  Worker 実装では 503 を `TheearthClientError` として扱いつつ、数回のリトライを
  検討する価値がある。
- **未検証・要確認**: 表示条件 (F-GOS0030) を実際に適用してから一覧の反映を確認する
  postback、单一運行ではなく複数選択時の zip 命名規則 (今回のファイル名は
