...
3. **テンプレの日付は固定ではなく `要素` シート駆動**。Y時間 の A 列は
   ...
   **訴訟準備の出力タブ (`/litigation`、#1133 c1133-2) は body の `period_rewrite: true` で
   区切りごとに期間を振り直す** (`writeYTimeRows` の `period`: `要素!F3`/`I3`・Y時間 A 列の
   キャッシュ値・`月所!B6`)。`月所!B6` は `要素!F3` を参照しない値の直書きなので別に書く。
   `/y-time-export` ページは振り直さない (テンプレ自前の期間のまま)。
...
- **最低賃金チェックの並びと右端の突合ブロック** (ユーザー決定 2026-07-30、
  ...
  **法定外休日 (祝日・会社指定休の出勤) の列は該当者が居る月だけ出す** (Refs #566) —
  差分列の検算は深夜(通常) 以外の 8 項すべてを引くので、0 以外は日別データの不整合を指す。
  **式は relay (`restraint-wage.ts` の `unaccountedMinutes`) にだけある** — 画面は応答の
  `invariants` を表示するだけ。3 条件の一覧は「検証」タブ (Refs #1123)。**`invariants` は
  拘束時間ソースが GCP のときだけ付く** — 現行では差分列は「-」(灰) で、検証タブは表を出さない。
  条件3 は**同じ乗務員の勤務 (GCP `kintai.shifts`) の時間帯の重なり** (relay
  `GET /kintai-relay/shift-overlaps`)。日の拘束の合計 > 24h では見ない (合計 20h のかぶりを見逃すため)
