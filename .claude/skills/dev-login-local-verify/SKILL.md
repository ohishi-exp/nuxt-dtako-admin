...
**cwd がその worktree の workerd だけ**をゾンビとみなす (`[ "/proc/$pid/cwd" -ef "$WT" ]`。
文字列比較にしないのは、archive 済みセッションの孤児 workerd の cwd が
`... (deleted)` 付きで返るため。2026-08-27 実測で 30 個中 28 個がこの孤児だった)。
**持ち主を判定できない workerd は warn を出して素通しする** — 本当の衝突は port の
先住チェックが捕まえるので、二次的なヒューリスティックで起動を阻む方が害が大きい。

> **★ この案内文を書き換えるときは `pkill` を二重引用符の中に置かないこと。**
> バッククォート付きで `echo "… \`pkill -x workerd\` …"` と書くとコマンド置換として
> **実行され、兄弟セッションの workerd を全部 kill する**。2026-08-27 に実際にやって
> 30 個巻き込んだ。この行は単一引用符の `echo '…'` にしてある。
